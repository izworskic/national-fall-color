...
html = html.replace('<div class="eyebrow">Blue Ridge Parkway · elevation-aware foliage intelligence</div>', '<div class="eyebrow">Blue Ridge Parkway · fall color trip planner</div>');
...
const planner = `<div class="persona-planner" data-persona-planner="true">
  ...
  <div class="persona-row" role="group" aria-label="Parkway area">
    ...
    <div class="persona-buttons">
      <button type="button" data-focus="all" aria-pressed="true">Anywhere / I’m flexible</button>
...
html = html.replace('<span>evidence coverage</span>', '<span>evidence used</span>');
...
(function(){
 ...
 function depth(best,offset){const original=offset?data?.decision?.best_next_7d:data?.decision?.best_now;return original?.id===best?.id?"Timing + conditions":"Timing + road"}
